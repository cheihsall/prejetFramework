...
//Debut créationn de la page de connexion
    ...
    {
        $request->validate([ // creation d'une variable reponse puis intégration de la méthode validate pour acepter si les règle de validation sont accepté oui oou  non
            ////////////
            'email' => ['required', 'email', 'regex: /^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/'],//email de la bd pour controler que le champ est obigatoire ,que l'email valid,écrit correctement
            'passwords' => 'required', 'string', //////////mdp de la bd pour controler que le champ est obigatoire 
        ]);
            ...
            if ($user->email == $request->get("email") && $user->motdepasse == $request->get("passwords")) { //controler la présence du mot de passe et de l'email
                if ($user->etat === "1") { //Voir si le user qui veut se connecter est archivé ou pas à partir de son etat
                    ...
                    if ($user->role === "administrateur") { // Redirection à la page admin à partir du role
                        ...
                        $_SESSION['prenom'] = $user->prenom;//affichage ge nom 
                        $_SESSION['photo'] = $user->photo;//affichage nom 
                        $_SESSION['prenom'] = $user->prenom;//affichage nom 
