    ...
    <header>
        <nav class="navbar navbar-expand-lg bg-success p-4">
            ...
            <form class="d-flex" role="search" action="recherche" method="post">
                <input class="form-control me-2" id="recherche" name="prenom"  value="{{ request()->prenom ?? ''}}" type="search" onchange="search()" placeholder="rechercher par prenom" aria-label="Search">
                <button class="btn btn-outline-light p-1" id="recherche" type="submit">rechercher</button>
            </form>&nbsp;
            <div class="nav-item mb-3 p-2" >
                <a href="/api/posts">
                <button type="button" id="quit" onchange="search()" class="btn btn-outline-danger mt-3 p-1 "  onchange="search()" style="display:none">
